...
function [mu,S,C,X,Y,w] = unscentedTransformation(M,P,g,g_param,tr_param)

    if nargin < 3
        errorMessage('unscentedTransformation needs at least 3 arguments');
        return;
    end
    ...
    elseif mat == 1
        [WM,W,c] = calcWeightsMatrix(size(M,1),alpha,beta,kappa);
        X = calcSigmaPoints(M,P,c);
        w = {WM,W,c};
    else
        [WM,WC,c] = calcWeights(size(M,1),alpha,beta,kappa);
        X = calcSigmaPoints(M,P,c);
        w = {WM,WC,c};
    end
    ...
    if isnumeric(g)
        Y = g*X;
    elseif ischar(g) | isa(g,'function_handle')
        Y = [];
        for i=1:size(X,2)
